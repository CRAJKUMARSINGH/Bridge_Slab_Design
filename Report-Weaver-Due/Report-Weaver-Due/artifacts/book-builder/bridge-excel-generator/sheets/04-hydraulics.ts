...
/**
 * Excel row of the HYDRAULICS sheet TOTAL line (Σ area in column F, Σ perimeter in column G).
 * Layout: rows 1–5 fixed, cross-section segments start at row 6, blank row, then TOTAL.
 * Must match generateHydraulicsSheet exactly.
 */
export function getHydraulicsTotalRow(crossSectionPointCount: number): number {
  return 7 + crossSectionPointCount;
}

/**
 * Returns the exact row numbers for critical hydraulic summary lines.
 * Useful for other sheets (Pier/Abutment) to link formulas dynamically.
 */
export function getHydraulicsSummaryRowRefs(crossSectionPointCount: number) {
  const totalRow = getHydraulicsTotalRow(crossSectionPointCount);
  const aRow = totalRow + 2;        // Area A
  const pRow = totalRow + 3;        // Perimeter P
  const rRow = totalRow + 4;        // Radius R
  const nRow = totalRow + 5;        // Manning n
  const sRow = totalRow + 6;        // Bed slope S
  const vRow = totalRow + 7;        // Velocity V
  const qRow = totalRow + 8;        // Discharge Q
  const rtlRow = totalRow + 17;     // RTL in summary
  const nblRow = totalRow + 20;     // NBL in summary
  const flRow = totalRow + 22;      // FL in summary
...
export async function generateHydraulicsSheet(
  ...
  // Summary calculations (cached values = design engine / Excel evaluation)
