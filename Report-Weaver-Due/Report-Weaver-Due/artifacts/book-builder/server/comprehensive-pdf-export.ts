...
async function generateComprehensivePDFInternal(
  ...
  // Foreword (page 2) — STRUDS-style "About this report"
  doc.addPage();
...
/**
 * STRUDS-style cover page (parity with client {@link StrudsCoverPage}).
 * Gradient header band, firm bar, IRC code-chip strip, "FOR SUBMISSION"
 * stamp, eyebrow + large title, project meta block, signature triplet.
 */
function addCoverPage(doc: jsPDF, input: EnhancedProjectInput, totalPages: number): void {
  const bridgeTypeLabel = input.bridgeType === 'high-level' ? 'High-Level RCC Slab Bridge' : 'Submersible RCC Slab Bridge';
  const PW = PAGE_WIDTH;
  const PH = PAGE_HEIGHT;

  // Page background subtly tinted
  doc.setFillColor(248, 250, 253);
  doc.rect(0, 0, PW, PH, 'F');

  // ── Top gradient band (deep navy → mid teal) — emulated by stacked rects ──
  const bandTop = 0;
  const bandH = 55;
  const bandSteps = 18;
  for (let s = 0; s < bandSteps; s++) {
    const t = s / (bandSteps - 1);
    const r = Math.round(31 + t * (15 - 31));
    const g = Math.round(73 + t * (90 - 73));
    const b = Math.round(107 + t * (140 - 107));
    doc.setFillColor(r, g, b);
    doc.rect(0, bandTop + (bandH * s) / bandSteps, PW, bandH / bandSteps + 0.5, 'F');
  }

  // Firm name (left of band)
  doc.setTextColor(255, 255, 255);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  const firmName = (input as any).firmName || 'Consulting Bridge Engineers';
  doc.text(firmName.toString().toUpperCase(), MARGIN, 22);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(220, 230, 245);
  doc.text('Structural Design · IRC Compliance · Independent Review', MARGIN, 28);

  // "Structural Design Report" eyebrow on right of band
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8);
  doc.setTextColor(255, 255, 255);
  doc.text('STRUCTURAL DESIGN REPORT', PW - MARGIN, 22, { align: 'right' });
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(220, 230, 245);
  doc.setFontSize(7);
  doc.text(`Issued ${new Date().toLocaleDateString('en-IN', { day: '2-digit', month: 'long', year: 'numeric' })}`, PW - MARGIN, 28, { align: 'right' });

  // ── IRC code-chip strip (just below the band) ──
  const chipY = bandH + 8;
  const chips = ['IRC:6-2017', 'IRC:78-2014', 'IRC:112-2011', 'IRC:SP:13', 'IS:1893-2016'];
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(7);
  let chipX = MARGIN;
  chips.forEach((chip) => {
    ...
    doc.roundedRect(chipX, chipY, w, 6, 1, 1, 'FD');
    doc.setTextColor(31, 73, 107);
    doc.text(chip, chipX + 3, chipY + 4.2);
    chipX += w + 4;
  });

  // ── "FOR SUBMISSION" rotated stamp (top-right area) ──
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(192, 60, 50);
  doc.setDrawColor(192, 60, 50);
  doc.setLineWidth(0.6);
  const stampX = PW - 28;
  const stampY = 90;
  doc.roundedRect(stampX - 22, stampY - 8, 44, 14, 1.5, 1.5, 'D');
  doc.text('FOR SUBMISSION', stampX, stampY + 1.5, { align: 'center', angle: -8 });
  ...
  doc.text('PROJECT', MARGIN, titleY);

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(28);
  doc.setTextColor(20, 45, 75);
  doc.text(bridgeTypeLabel, MARGIN, titleY + 14);

  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(80, 100, 130);
  const subtitle = `${input.numberOfSpans === 1 ? 'Single-Span' : `${input.numberOfSpans}-Span`} · ${input.totalLength} m total · ${input.riverName || '—'}`;
  doc.text(subtitle, MARGIN, titleY + 22);

  // ── Project name highlight block ──
  const nameY = titleY + 32;
  doc.setFillColor(232, 240, 250);
  doc.rect(MARGIN, nameY, CONTENT_WIDTH, 16, 'F');
  doc.setDrawColor(180, 200, 225);
  doc.setLineWidth(0.3);
  doc.line(MARGIN, nameY, MARGIN, nameY + 16);
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7);
  doc.setTextColor(100, 130, 165);
  doc.text('NAME OF WORK', MARGIN + 4, nameY + 5);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(20, 45, 75);
  doc.text(input.projectName, MARGIN + 4, nameY + 11.5);

  // ── Project meta grid (2 × 3) ──
  const metaY = nameY + 22;
  const colW = CONTENT_WIDTH / 4;
  const metaItems: [string, string][] = [
    ['LOCATION',  input.location || '—'],
    ['CLIENT',    (input as any).clientName || (input as any).client || '—'],
  ...
  metaItems.forEach(([k, v], idx) => {
    ...
    doc.setFontSize(10);
    doc.setTextColor(20, 45, 75);
    const trimmed = doc.splitTextToSize(String(v), colW - 4);
    doc.text(trimmed, x, metaY + 6);
  });

  // ── Signature triplet at the foot ──
  const sigY = PH - 50;
  const sigW = (CONTENT_WIDTH - 20) / 3;
  const roles: [string, string, string][] = [
    ['DESIGNED BY', (input as any).designerName || 'Design Engineer', 'M.E. (Civil) · Structural'],
    ['CHECKED BY', '_________________________', 'Senior Engineer'],
    ['APPROVED BY', '_________________________', 'Executive Engineer · PWD'],
  ];
  roles.forEach((r, idx) => {
    const x = MARGIN + idx * (sigW + 10);
    doc.setDrawColor(31, 73, 107);
    doc.setLineWidth(0.3);
    doc.line(x, sigY + 12, x + sigW, sigY + 12);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    doc.setTextColor(100, 130, 165);
    doc.text(r[0], x, sigY + 17);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(20, 45, 75);
    doc.text(r[1], x, sigY + 23);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.setTextColor(100, 130, 165);
    doc.text(r[2], x, sigY + 28);
  });
  ...
  doc.text(`Page 1 of ${totalPages}  ·  ${input.projectName}`, MARGIN, PH - 7);
  doc.text('Generated by Bridge Design System', PW - MARGIN, PH - 7, { align: 'right' });
}

/**
 * Foreword / About this report (parity with client {@link StrudsForeword}).
 * Lead paragraph derived from inputs, scope grid (6 cells), governing-codes
 * strip, deliverables list. All values pulled straight off `input`.
 */
function addForeword(doc: jsPDF, input: EnhancedProjectInput, totalPages: number): void {
  ...
  // Header band
  doc.setFillColor(31, 73, 107);
  doc.rect(0, 0, PW, 22, 'F');
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(255, 255, 255);
  doc.text('FOREWORD · ABOUT THIS REPORT', MARGIN, 14);
  doc.setFontSize(8);
  doc.setFont('helvetica', 'normal');
  doc.text(`Page ${pageNum} of ${totalPages}`, PW - MARGIN, 14, { align: 'right' });

  // Eyebrow + title
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8);
  doc.setTextColor(100, 130, 165);
  doc.text('FOREWORD', MARGIN, 34);
  ...
  const lead =
    ...
    'It transitions in measured sequence from the catchment hydraulics — discharge, regime width, scour ' +
    'and afflux — into the structural design of the deck slab, piers and abutments, closing with quantities ' +
    'and an audit-ready bill of quantities. Every value presented is computed from the inputs on the previous ' +
    'page; no figure is editorial. The intent is a single self-contained submission an approving authority can ' +
    'read end-to-end and reproduce on first principles.';
  const leadLines = doc.splitTextToSize(lead, CONTENT_WIDTH);
  doc.text(leadLines, MARGIN, 53);

  // ── Scope grid (2 cols × 3 rows) ──
  const scopeY = 53 + leadLines.length * 4.5 + 8;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(20, 45, 75);
  doc.text('Scope of work', MARGIN, scopeY);

  const cellW = (CONTENT_WIDTH - 6) / 2;
  const cellH = 22;
  const cells: [string, string][] = [
    [
      'Bridge configuration',
      `${input.numberOfSpans} spans of ${input.spanLength.toFixed(1)} m c/c (effective length ${input.totalLength.toFixed(1)} m), ` +
        `carriageway ${input.carriageWidth.toFixed(1)} m, overall width ${(((input as any).totalWidth ?? input.carriageWidth + 1.5) as number).toFixed(1)} m.`,
    ],
    [
      'Hydraulic design',
      `Discharge by Manning's formula on surveyed cross-section; Lacey regime width, scour by ASTRA factor, ` +
        `Molesworth afflux. HFL ${input.hfl.toFixed(2)} m, bed RL ${input.bedLevel.toFixed(2)} m.`,
    ],
    [
      'Stability',
      `Pier and abutment stability for all governing IRC load combinations including seismic, hydrodynamic and ` +
        `braking; SBC ${(input.sbc ?? 0).toFixed(0)} kN/m², phi ${(input.phi ?? 30).toFixed(0)} deg.`,
    ],
    [
      'Structural design',
      'Deck slab IRC:112 limit state, pier/abutment reinforcement, footing design and foundation pressure check, ' +
        'deck anchorage and expansion arrangement.',
    ],
    [
      'Drawings',
      'GAD, pier and abutment elevations, slab reinforcement, hydraulic profile and cross-section overlays.',
    ],
    [
      'Bill & abstract',
      'Item-wise BOQ for earthwork, PCC, structural concrete, steel, pitching and ancillaries with rate analysis.',
    ],
  ];
  ...
  cells.forEach((c, idx) => {
    ...
    doc.setFontSize(8);
    doc.setTextColor(60, 75, 100);
    const lines = doc.splitTextToSize(c[1], cellW - 6);
    doc.text(lines.slice(0, 4), x + 3, y + 10);
  });

  // ── Governing codes strip ──
  const codesY = scopeY + 6 + 3 * (cellH + 4) + 6;
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(11);
  doc.setTextColor(20, 45, 75);
  doc.text('Governing codes & references', MARGIN, codesY);
  ...
  codeChips.forEach((chip) => {
    ...
    doc.setDrawColor(180, 200, 225);
    doc.setLineWidth(0.2);
    doc.roundedRect(cx, cy, w, 6, 1, 1, 'FD');
    doc.setTextColor(31, 73, 107);
    doc.text(chip, cx + 3, cy + 4.2);
    cx += w + 4;
  });
  ...
  doc.setFontSize(11);
  doc.setTextColor(20, 45, 75);
  doc.text('Deliverables in this report', MARGIN, dlvrY);
  ...
  let ly = dlvrY + 8;
  deliverables.forEach(([head, body]) => {
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(9);
    doc.setTextColor(31, 73, 107);
    doc.text('•', MARGIN, ly);
    doc.text(head, MARGIN + 4, ly);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    doc.setTextColor(60, 75, 100);
    const lines = doc.splitTextToSize(body, CONTENT_WIDTH - 36);
    doc.text(lines, MARGIN + 36, ly);
    ly += Math.max(5, lines.length * 4.2);
  });
  ...
  doc.setFontSize(7);
  doc.setTextColor(120, 140, 165);
  doc.text(`${input.projectName} · Foreword`, MARGIN, PH - 7);
  doc.text(`Page ${pageNum} of ${totalPages}`, PW - MARGIN, PH - 7, { align: 'right' });
}
